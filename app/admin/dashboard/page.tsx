...
export default function AdminDashboardPage() {
  ...
  const statCards = stats ? [
    ...
    { label: 'Monthly Bookings', value: stats.monthlyBookings, icon: TrendingUp, color: 'text-gold-500 bg-gold-100 dark:bg-[#231B12]/30' },
      ...
      <div className="vintage-container py-8">
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <Link href="/admin/cms" className="px-3 py-1.5 rounded-lg bg-white dark:bg-[#231B12] border border-border dark:border-white/10 text-xs text-foreground dark:text-[#F3ECDA] hover:border-forest-400 transition-colors">
              ...
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2.5 text-sm font-sans font-medium border-b-2 transition-all ${
                  currentTab === tab.id
                    ...
                    : 'border-transparent text-muted-foreground dark:text-[#B8A385] hover:text-foreground'
                }`}
              >
        ...
        {currentTab === 'overview' && (
          <>
            {loading ? (
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {[1, 2, 3, 4, 5, 6, 7, 8].map((i) => (
                  <div key={i} className="glass-card-light dark:bg-[#231B12] animate-pulse p-5 rounded-2xl">
                    <div className="h-4 bg-gold-100 dark:bg-[#2E2519] rounded w-1/2 mb-3" />
                    <div className="h-8 bg-gold-100 dark:bg-[#2E2519] rounded w-1/3" />
                  </div>
                ))}
              </div>
            ) : (
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {statCards.map((card) => (
                  <ScrollReveal key={card.label} delay={0.05}>
                      <div className="glass-card-light dark:bg-[#231B12] rounded-2xl p-5">
                        <div className={`w-10 h-10 rounded-xl ${card.color} flex items-center justify-center mb-3`}>
                          <card.icon className="w-5 h-5" />
                        </div>
                        <p className="text-muted-foreground dark:text-[#B8A385] text-xs mb-1">{card.label}</p>
                        <p className="text-2xl font-bold text-foreground dark:text-[#F3ECDA]">{card.value}</p>
                    </div>
                  </ScrollReveal>
                ))}
              </div>
            ...
            <div className="mt-8 grid md:grid-cols-2 gap-6">
              <ScrollReveal>
                <div className="glass-card-light dark:bg-[#231B12] rounded-2xl p-6">
                  <h2 className="font-serif text-lg text-foreground dark:text-[#F3ECDA] mb-4">Quick Actions</h2>
                  <div className="space-y-3">
                    {[
                      ...
                      <Link
                        key={item.label}
                        href={item.href}
                        className="block p-3 rounded-xl bg-earth-50/80 dark:bg-[#2E2519] hover:bg-gold-50 dark:hover:bg-[#2E2519] transition-colors"
                      >
                        <p className="text-sm font-medium text-foreground dark:text-[#F3ECDA]">{item.label}</p>
                        <p className="text-xs text-muted-foreground dark:text-[#B8A385]">{item.desc}</p>
                      </Link>
                    ))}
                  </div>
                </div>
              ...
              <ScrollReveal delay={0.1}>
                <div className="glass-card-light dark:bg-[#231B12] rounded-2xl p-6">
                  <h2 className="font-serif text-lg text-foreground dark:text-[#F3ECDA] mb-4">Activity Log</h2>
                  <p className="text-muted-foreground dark:text-[#B8A385] text-sm">View recent system activity and changes.</p>
                  {stats && (
                    <div className="mt-4 space-y-2">
                      <div className="flex justify-between text-sm">
                        <span className="text-muted-foreground dark:text-[#B8A385]">Active Cottages</span>
                        <span className="font-medium dark:text-[#F3ECDA]">{stats.activeCottages} / {stats.totalCottages}</span>
                      </div>
                      <div className="flex justify-between text-sm">
                        <span className="text-muted-foreground dark:text-[#B8A385]">Pending Bookings</span>
                        <span className="font-medium dark:text-[#F3ECDA]">{stats.pendingBookings}</span>
                      </div>
                      <div className="flex justify-between text-sm">
                        <span className="text-muted-foreground dark:text-[#B8A385]">Unread Messages</span>
                        <span className="font-medium dark:text-[#F3ECDA]">{stats.unreadMessages}</span>
                      </div>
                    </div>
                  )}
