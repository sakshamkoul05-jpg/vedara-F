...
export default function EmployeeDashboardPage() {
  ...
  return (
    <div className="min-h-screen bg-alabaster dark:bg-[#140D07] pt-24">
      <div className="vintage-container py-6">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="font-serif text-xl text-foreground dark:text-[#F3ECDA] leading-tight">{user?.name}</h1>
...
className={`px-5 py-2 rounded-full text-sm font-sans font-medium transition-all duration-300 ${
               tab === 'bookings'
                 ...
                 : 'bg-gold-50 dark:bg-[#2E2519] text-charcoal/70 dark:text-[#B8A385] hover:bg-gold-100'
             }`}
           >
        ...
        {tab === 'bookings' && (
          <div>
            <div className="flex items-center gap-2 mb-5">
              ...
              <h2 className="font-serif text-lg text-foreground dark:text-[#F3ECDA]">Today&apos;s Bookings</h2>
            </div>
            {loading ? (
              ...
              <div className="space-y-4">
                {bookings.map((booking: any, i: number) => (
                  <ScrollReveal key={booking.id} delay={i * 0.05}>
                    <div className="backdrop-blur bg-white/60/60 dark:bg-[#231B12] border border-white/20 dark:border-white/10 rounded-xl p-5 shadow-sm hover:shadow-md transition-all duration-300 font-sans">
                      <div className="flex items-start justify-between">
                        <div className="space-y-1.5 flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            <span className="font-medium text-foreground dark:text-[#F3ECDA] text-sm truncate">
                              {booking.guest?.name}
                            </span>
                          ...
                          <p className="text-xs text-muted-foreground dark:text-[#B8A385] flex items-center gap-1.5">
                          ...
                          <p className="text-xs text-muted-foreground dark:text-[#B8A385]">
        ...
        {tab === 'orders' && (
          <div>
            <div className="flex items-center gap-2 mb-5">
              ...
              <h2 className="font-serif text-lg text-foreground dark:text-[#F3ECDA]">Order Queue</h2>
            </div>
            {loading ? (
              <div className="flex justify-center py-12">
                <Loader2 className="w-6 h-6 text-gold-600 animate-spin" />
                    ...
                    <div className="backdrop-blur bg-white/60/60 dark:bg-[#231B12] border border-white/20 dark:border-white/10 rounded-xl p-5 shadow-sm hover:shadow-md transition-all duration-300 font-sans">
                      <div className="flex items-start justify-between mb-4">
                        <div>
                          <div className="flex items-center gap-2 mb-1">
                            <span className="font-medium text-foreground dark:text-[#F3ECDA] text-sm">Table {order.tableNumber}</span>
                          ...
                          <p className="text-xs text-muted-foreground dark:text-[#B8A385]">{order.orderRef}</p>
                          {order.guestName && (
                            <p className="text-xs text-muted-foreground dark:text-[#B8A385] mt-0.5">{order.guestName}</p>
                          )}
                        </div>
                      ...
                      <div className="space-y-1.5 mb-4 pb-4 border-b border-earth-100 dark:border-white/10">
                        {order.items?.map((item: any) => (
                          <div key={item.id} className="flex justify-between text-xs text-muted-foreground dark:text-[#B8A385]">
